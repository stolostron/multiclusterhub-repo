...
if [ "${TRAVIS_BRANCH}" != "master" ] && [[ "${TRAVIS_BRANCH}" != "release-"* ]] && [[ "${TRAVIS_BRANCH}" != "dev-"* ]]; then
    git clone https://${GITHUB_USER}:${GITHUB_TOKEN}@github.com/stolostron/multiclusterhub-repo.git
    ...
    git merge origin/release-2.2 -m "[skip-chart-sync] resolve conflicts" -s recursive -X ours
